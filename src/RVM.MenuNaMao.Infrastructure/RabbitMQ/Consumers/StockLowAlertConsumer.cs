...
public sealed class StockLowAlertConsumer(IOptions<RabbitMqSettings> settings, ILogger<StockLowAlertConsumer> logger)
    : BackgroundService
{
    private const string QueueName = "stock.low-alert";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            consumer.ReceivedAsync += async (_, ea) =>
            {
                ...
                logger.LogWarning("[{Queue}] ALERT: {Message}", QueueName, body);
